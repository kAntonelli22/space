...
func _process(_delta):
   ...
   if movement_points > 0 and !is_moving:
      print("cpu: ship moving")
      ai_move()
   elif movement_points == 0 and !is_moving:
      print("cpu: ship deselecting self")
      is_selected = false
      SignalBus.emit_signal("turn_complete", self)
